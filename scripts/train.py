...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='pascal_voc',
                        choices=['pascal_voc', 'pascal_aug', 'ade20k', 'citys', 'sbu', 'my_city'],
...
class Trainer(object):
    ...
    def train(self):
        ...
        plot_loss_curve(losses_list, iterations_list, self.args)
...
# 添加一个函数用于绘制损失曲线
def plot_loss_curve(losses, iterations, args):
    import matplotlib.pyplot as plt

    # 从 args 提取训练相关参数（可以根据实际 args 结构调整）
    epochs = args.get('epochs', 'Unknown Epochs')  # 训练轮次
    learning_rate = args.get('lr', 'Unknown LR')  # 学习率
    batch_size = args.get('batch_size', 'Unknown Batch Size')  # 批次大小
    ...
    # 设置标题，包含训练参数信息
    plt.title(f"Training Loss Curve | Epochs: {epochs}, LR: {learning_rate}, Batch Size: {batch_size}")

    plt.legend()
    plt.grid(True)

    # 设置动态图片名称
    image_name = f"training_loss_curve_epochs{epochs}_lr{learning_rate}_batch{batch_size}.png"
...
def save_checkpoint(model, args, is_best=False):
    ...
    filename = '{}_{}_{}_{}_{}.pth'.format(args.model, args.backbone, args.dataset, args.lr, args.epochs)
    ...
    if is_best:
        best_filename = '{}_{}_{}_{}_{}_best_model.pth'.format(args.model, args.backbone, args.dataset, args.lr, args.epochs)
...
if __name__ == '__main__':
    ...
    if args.distributed:
        ...
        # synchronize()
        local_rank = int(os.environ["LOCAL_RANK"])  # 从环境变量中读取 LOCAL_RANK
